...
| log_analytics_retention_days | Retention period in days (30-730) | `number` | `30` | no |
| application_insights_type | Application type (web, other, etc.) | `string` | `"web"` | no |
...
| log_analytics_primary_shared_key | Primary shared key for Log Analytics | yes |
...
- Log Analytics uses Pay-As-You-Go pricing (PerGB2018 SKU)
- Retention period affects costs - longer retention = higher cost
- Application Insights data is stored in Log Analytics Workspace
- Default retention is 30 days (minimum)
- Maximum retention is 730 days (2 years)
